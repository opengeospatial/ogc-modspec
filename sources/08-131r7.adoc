= The ModSpec Model - Part 1: Core - A Standard for Designing and Writing Modular Standards
:doctype: standard
:encoding: utf-8
:lang: en
:status: approved
:committee: technical
:draft: 3.0
:doc-type: POL
:standard: ModSpec-part1
:edition: 1.1
:external-id: http://www.opengis.net/doc/{doc-type}/{standard}/{edition}
:docnumber: 08-131r7
:received-date: 2025-10-20
:issued-date: 2026-03-05
:published-date: 2026-05-21
:fullname: Carl Reed
:fullname_2: Charles Heazel
:fullname_3: John Herring
:docsubtype: logical-model
:keywords: ogcdoc, OGC document, modspec, modular, requirements, testing, core
:submitting-organizations: Carl Reed & Associates; Heazeltech; Tepa Enterprise Solutions (ImageMatters); The Danish Agency for Climate Data; UK Met Office
:imagesdir: images
:mn-document-class: ogc
:mn-output-extensions: xml,html,pdf
:local-cache-only:
:data-uri-image:
:pdf-uri: ./08-131r7.pdf
## :xml-uri: ./document.xml
## :doc-uri: ./document.doc


include::sections/00-preface.adoc[]

include::sections/00-foreword.adoc[]

include::sections/01-scope.adoc[]

include::sections/02-conformance.adoc[]

include::sections/03-norm-refs.adoc[]

include::sections/04-terms-and-defs.adoc[]

include::sections/05-conventions.adoc[]

include::sections/06-fundamentals.adoc[]

include::sections/06-req-class.adoc[]

include::sections/07-mapping.adoc[]

include::sections/aa-abstract-conformance.adoc[]

include::sections/ab-changes-required.adoc[]

include::sections/ad-bibliography.adoc[]

include::sections/ae-acknowledgements.adoc[]
