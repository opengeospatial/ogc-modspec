...
The OGC ModSpec is a framework for designing and structuring geospatial standards to enhance consistency, testability, and usability. 
The ModSpec mandates a model-driven approach where standards are "broken" into reusable requirements classes — often called building blocks — to enable incremental 
implementation, ranging from core functionality to advanced extensions. 

More specifically, the ModSpec is an OGC member developed and approved document that defines a model and related requirements 
...
The goals of using the ModSpec are as follow.
...
- Consistently stated and modular requirements can be used as components (building blocks).
- Well crafted requirements that are consistently documented, such as using the OGC Metanorma document template, can much more easily be harvested and used in other applications, such as AI based coding tools.
...
This document also uses the terms as defined in Clause 4: Terms and Definitions as specified in the OGC ModSpec Model - Part 1: Core - A Standard for Designing and Writing Modular Standards 
