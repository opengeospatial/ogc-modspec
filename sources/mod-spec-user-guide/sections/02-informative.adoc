...
A `requirements package` ia a set (grouping) of related requirement classes and their associated components. Quite often, a standard may have related requirements classes, recommendations, and permissions of the same target type. An example is the https://github.com/opengeospatial/cdbswg/blob/master/cdb-2.0/23-034/sections/cdb-core-crs-requirements-package.adoc[CDB 2.0 Coordinate Reference System Requirements Package]. The CDB 2.0 CRS Requirements Package consists of two requirements classes, recommendations and permissions. 
...
The following is an example of an AsciiDoc coding following the Metanorma publication rules. This example is a snippet  of the ModSpec 1.1 core conformance class. The example is consistent with the Requirements Class example shown above.
