...
= Key ModSpec concepts with AsciiDoc/Metanorma examples

This ModSpec User Guide describes the key concepts defined in the OGC ModSpec Standard (Version 1.1). Further, this document provides AsciiDoc/Metanorma examples of how to code
...
image::../../images/ModSpec_diagram 2026.jpg[ModSpec]
...
Every new OGC Standard or revision to an OGC Standard must state a `standardization goal`. This is a concise statement of the 
...
In the above standardization goal example, the target type is either a `conceptual model` or a `logical model`. 
...
target subtypes that derive from the root superclass. For example, the OGC API - Features Standard has a target type of `WebAPI`. There can then
subtypes such as WebAPI/landing-page and WebAPI/crs or branches such as client and server. 

Why the target type concept is important is shown below in the discussion (below) on `requirement`. 
...
be structured per the rules specified in the OGC Naming Authority policy document https://docs.ogc.org/pol/10-103r1.html[OGC Name Type Specification - specification elements]
...
identify the CRS. The CRS_SHALL be referenced by a uniform resource identifier (i.e., a URI)  
...
In the OGC, requirements are  labeled and have a unique identifier (URI). So the above CRS example could be written as below and be compliant with the requirements
of the ModSpec
...
An implementation SHALL specify a coordinate reference system (CRS). An EPSG code SHALL be used to 
identify the CRS. The CRS SHALL be referenced by a uniform resource identifier (i.e., a URI)  
such as http://www.opengis.net/def/crs/EPSG/0/4326
```
...
==== OGC Traditional AsciiDoc coding example
...
^| B | An EPSG code SHALL be used to identify the CRS. 
...
part:: An EPSG code SHALL be used to identify the CRS.
...
The ModSpec defines a `recommendation` as an expression, in the content of a standard, that conveys a suggested possible choice or course of action deemed to be particularly suitable without necessarily mentioning or excluding others. A recommendation is indentified by the use of the verb `SHOULD`. An example of a recommendation is:
...
A recommendation could be general and applicable to statements in the entire standards, such as `HTML SHOULD be used to implement this standard`. A recommendation could also be applicable to just one requirement or requirements class.

Note that in the negative form, a recommendation is the expression that a suggested possible choice or course of action is not preferred but it is not prohibited. Also note - and this is important - recommendations are not tested and therefore have no related conformance test. Therefore, a recommendation CANNOT be stated as part of a requirements class. This is because a requirements class is testable. Recommednation(s) can be included in a `requirements package` (see below).

=== Metanorma coding example for Recommendation

The following is an example of how a recommendation is expressed using the OGC Metanorma template. The recommendation shown below is a global ModSpec recommendation for all elements specified in a standards document.
...
=== Publication of a recommendation using Metanorma template

The above codeing example would be published as follows:
...
target:: /req/core/all-components-assigned-uri
test-purpose:: Validate that each component of a standard, including requirements, requirements packages, requirements classes, 
conformance test, conformance packages, and conformance test classes are assigned a unique identifier or label.
test-method:: Inspect the document to verify the above.
====
...
most OGC standards that state requirements for how to specify and/or use coordinate reference systems specify a dependency
...
Key Aspects of OGC ModSpec include:

- Structure: Defines strict rules for organizing standards into modules, including requirements, recommendation classes, and conformance test cases.
- Purpose: Ensures that standards are easy to implement, modular, and testable for high interoperability.
- Model-Driven: Standards are developed using formal models, which are often translated into machine-readable formats.
- Requirements Classes: Standards are divided into small, manageable units that allow for targeted implementation (e.g., core vs. extensions).
