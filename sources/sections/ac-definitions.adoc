...
. a {{requirement}} is associated to at least one {{conformance test}}.
...
all on the same {{standardization target type}}.
...
. a {{conformance test class,conformance (test) class}} is also collection of
{{conformance test module,conformance test modules}} that group
...
implementations*. If a specification addresses multiple {{standardization target
type,standardization target types}}, it may have a *core* for each *target
type*.

. an *extension* of a {{requirements class}} is a second {{requirements class}}
...
must identify all of its dependencies during the execution of conformance tests
