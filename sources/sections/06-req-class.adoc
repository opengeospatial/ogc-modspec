...
[[req-1]]
include::../requirements/core/REQ_standardization-goal.adoc[]

include::../requirements/core/REQ_standardization-target-type.adoc[]

NOTE: These two requirements establish the context and scope of this Standard. Additional informative text would be useful.  

include::../requirements/core/REQ_ogc-compliance.adoc[]

NOTE: This requirement is for OGC work only and will be moved to the OGC Policy statement regarding the use of the ModSpec. This move will happen once the policy is removed.

[[req-1]]
include::../requirements/core/REQ_testable.adoc[]

This requirement states that every requirement is testable.
...
depend on some parameter that must be defined before the tests can be executed. This can be thought of as an "if-then-else" decision tree. 
