...
[[req-0]]
include::../requirements/core/REQ1_reqs-are-in-class.adoc[]
...
such part separately (see the definition of <<RequirementsClass-definition,requirements class>>). In this sense, the next requirement divides
...
separate standardization target type and thus belong to the requirements to be tested by
...
standardization target type). Implementors who wish to sacrifice modularity for some
